...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (isSignInWithEmailLink(auth, window.location.href)) {
        ...
        if (!email) {
            email = window.prompt('الرجاء إدخال بريدك الإلكتروني لتأكيد تسجيل الدخول:');
        }
        try {
            await signInWithEmailLink(auth, email, window.location.href);
            window.localStorage.removeItem('emailForSignIn');
            showToast("تم تسجيل الدخول بنجاح عبر الإيميل! 🚀");
            window.history.replaceState({}, document.title, window.location.pathname);
        } catch (error) {
            alert("خطأ في التحقق من الإيميل: " + error.message);
        }
    }
});
...
window.sendLoginLinkToEmail = async function() {
    const email = document.getElementById('login-email').value.trim();
    if (!email) {
        showToast("الرجاء إدخال البريد الإلكتروني في حقل الإيميل أولاً!");
        return;
    }
    ...
    try {
        ...
        showToast("تم إرسال رابط الدخول السريع إلى بريدك! ✉️ تفقد صندوق الوارد.");
    } catch (error) {
        alert("خطأ في إرسال الرابط: " + error.code);
    }
};
...
window.handleEmailLogin = async function(e) {
    ...
    if(!pass) {
        showToast("أدخل كلمة المرور أو اضغط على زر 'رابط الإيميل'");
        return;
    }
...
window.handleEmailRegister = async function(e) {
    ...
    try {
        ...
        // تسجيل الخروج حتى يفعل المستخدم حسابه عبر الإيميل
        await signOut(auth);

        showToast("تم إنشاء الحساب! ✉️ أرسلنا رابط تفعيل إلى بريدك، يرجى تفقده وتأكيده أولاً.");
    } catch (error) {
        alert("خطأ Firebase: " + error.code);
    }
};

onAuthStateChanged(auth, async (user) => {
    if (user) {
        // التحقق من تفعيل البريد الإلكتروني (ملاحظة: تسجيلات Google تكون مفعّلة تلقائياً)
        if (!user.emailVerified) {
            showToast("⚠️ حسابك غير مفعل! يرجى التأكيد عبر رابط التفعيل المرسل إلى بريدك.");
            await signOut(auth);
            return;
        }
...
async function syncUserToDatabase(user) {
    ...
    // تطبيق الثيم المحفوظ على الواجهة فوراً
...
window.fetchCloudMessages = async function() {
    ...
    try {
        ...
        // ترتيب الرسائل: الرسائل المثبتة تظهر دائماً في الأعلى
        messages.sort((a, b) => {
...
window.renderUsersList = function(users) {
    const listEl = document.getElementById('users-search-list');
    if(!listEl) return;
    ...
    listEl.innerHTML = users.map(user => `
        <div class="glass-card" style="display:flex; align-items:center; gap:12px; cursor:pointer;" onclick='openSendToUser(${JSON.stringify(user)})'>
...
window.filterUsers = function(queryText) {
    const q = queryText.toLowerCase().trim();
    const filtered = allUsersList.filter(u => u.name.toLowerCase().includes(q) || u.handle.toLowerCase().includes(q));
...
window.renderMessages = function() {
    ...
    if (!messages || messages.length === 0) {
        listEl.innerHTML = `<p style="text-align: center; color: var(--text-muted); padding: 20px;">لا توجد رسائل سحابية حالياً</p>`;
        return;
    }
