...
window.handleEmailRegister = async function(e) {
    ...
    try {
        ...
        // 1. إنشاء الحساب مباشرة في Firebase Auth
        const cred = await createUserWithEmailAndPassword(auth, email, pass);
        const user = cred.user;

        // 2. حفظ بيانات المستخدم في Firestore مباشرة
        const newUser = {
            uid: user.uid,
            name: name,
        ...
        showToast("تم إنشاء الحساب بنجاح! 🎉 مرحبا بك.");
...
async function syncUserToDatabase(user) {
    ...
    if (userSnap.exists()) {
        ...
        currentUser = {
            ...
            bio:"وااش عينيا أبعث(ي) مساج للكراش تاعك 👀",
...
window.fetchAllUsers = async function() {
    try {
        ...
        // نخلو قائمة النتائج فارغة تماماً عند فتح الصفحة
        const listEl = document.getElementById('search-results');
        if(listEl) listEl.innerHTML = `<p style="text-align: center; color: var(--text-muted); padding: 20px;">اكتب اسم الشخص للبحث عنه...</p>`;
...
window.handleSearch = function() {
    ...
    // إذا خانة البحث فارغة، لا تعرض شيئاً
    if (!queryText) {
        if(listEl) listEl.innerHTML = `<p style="text-align: center; color: var(--text-muted); padding: 20px;">اكتب اسم الشخص للبحث عنه...</p>`;
        return;
    }
...
window.downloadStoryImage = async function() {
    ...
    try {
        ...
        link.download = 'sarahne-story-hd.png';
...
// 🍎 تحويل عرض الإيموجيات إلى ستايل Apple
    ...
    const walker = document.createTreeWalker(
        ...
        NodeFilter.SHOW_TEXT,
        {
            acceptNode(node) {
                if (
                    !node.nodeValue.trim() ||
                    node.parentElement.closest("script, style, textarea, input")
                ) {
                    return NodeFilter.FILTER_REJECT;
                }

                return /[\p{Emoji_Presentation}\p{Extended_Pictographic}]/u.test(
                    node.nodeValue
                )
