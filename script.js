...
// 🟢 1. متغيرات EmailJS (تكون فارغة في الأول حتى تتحمل من Firebase)
let EMAILJS_PUBLIC_KEY = "BelT0PlpYzXma3XxB";
let EMAILJS_SERVICE_ID = "service_2h2rhme"; 
let EMAILJS_TEMPLATE_ID = "template_84nynjn"; 
...
async function loadEmailJSConfigFromFirebase() {
    try {
        ...
        // تفعيل EmailJS بالبيانات المجلوبة
        if (window.emailjs && EMAILJS_PUBLIC_KEY) {
            emailjs.init(EMAILJS_PUBLIC_KEY);
        }
    } catch (error) {
        console.error("خطأ في جلب إعدادات EmailJS من Firebase:", error);
    }
}
...
// 🟢 حفظ إعدادات EmailJS مباشرة في Firebase Firestore
window.saveEmailJSConfig = async function() {
    ...
    try {
        showToast("جاري الحفظ في Firebase... ⏳");

        // الحفظ في Firebase Firestore تحت كولكشن settings ووثيقة emailjs
        ...
        closeEmailJSConfig();
        showToast("تم حفظ الإعدادات بنجاح في Firebase! 🔥🚀");

    } catch (error) {
        console.error("خطأ الحفظ في Firebase:", error);
